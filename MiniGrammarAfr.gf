concrete MiniGrammarAfr of MiniGrammar = MiniCatAfr ** open Prelude, MiniResAfr in {
  ...
  lin
    ...
    ImpVP vp = {
      s = table {
        TPos => vp.v.s!VPres ++ vp.n2a ++ vp.n2b ++ vp.subCl ++ vp.compV!VInfa ++ vp.v.p ++ vp.adv ;
        TNeg => "moenie" ++ vp.n2a ++ vp.n2b ++ vp.subCl ++ vp.compV!VInfa ++ vp.v.s!VInfa ++ vp.adv ++ "nie"
      }
    } ; 
    ...
    ComplV2V v2v np vp = {
      ...
      inf = <(vp.v).s!VInfa,(vp.v).s!VPerf> ;
      vIsBe = vp.vIsBe ;
      n2a = case <np.isPron,v2v.hasC> of {
        <True,False> => v2v.c ++ np.s ! Acc ; -- hy sien [my] (nie) nie
        <_,_> => []
      } ;
      n2b = case <np.isPron,v2v.hasC> of {
        <True,False> => [] ;
        <_,_> => v2v.c ++ np.s ! Acc -- alle ander gevalle: hy kyk nie altyd [na my] nie; hy sien nie [die vrou] nie
      } ;
      subCl = vp.subCl ;
      adv = vp.adv ;
      filled = True ;
    ...
    -- { s : Case => Str ; a : Agr }
    i_Pron = regPron "ek" "my" Sg Per1 Neuter ;
    youSg_Pron = regPron "jy" "jou" Sg Per2 Neuter ;
    she_Pron = regPron "sy" "haar" Sg Per3 Fem ;
    he_Pron = mkPron "hy" "hom" "sy" Sg Per3 Fem ;
    ...
    we_Pron = regPron "ons" "ons" Sg Per1 Neuter ;
    youPl_Pron = regPron "julle" "julle" Sg Per2 Neuter ;
    they_Pron = regPron "hulle" "hulle" Sg Per3 Neuter ;
    it_NP = pronNP "dit" "dit" "sy" Sg Per3 Masc TPos ;

    no_one_NP = pronNP "niemand" "niemand" "niemand s'n" Sg Per3 Neuter TNeg ;
    nothing_NP = pronNP "niks" "niks" "niks s'n" Sg Per3 Neuter TNeg ;
    very_AdA = ss "baie" ;
