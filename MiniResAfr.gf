resource MiniResAfr = open Prelude,Predef in {
  ...
  param
    ...
    VType = VReg | VAux | VBe | VComp ; -- | VBeAux ;
    ...
    -- in building a clause, the final nie is always inserted for negative polarity, the initial nie depends on other factors
      ...
      regPron : Str -> Str -> Number -> Person -> Gender -> { s : Case => Str ; a : Agr } = \ek,my,num,per,g ->
      {
        s = table { Nom => ek ; Acc => my ; Gen => my } ;
        a = Ag num per g 
      } ;

      mkPron : Str -> Str -> Str -> Number -> Person -> Gender -> { s : Case => Str ; a : Agr } = \hy,hom,sy,num,per,g ->
      {
        s = table { Nom => hy ; Acc => hom ; Gen => sy } ;
        a = Ag num per g 
      } ;

      pronNP : Str -> Str -> Str -> Number -> Person -> Gender -> TPol ->
               {s : Case => Str ; a : Agr ; isPron : Bool ; p : TPol } =
      \hy,hom,sy,num,per,g,p ->
      {
        s = table { Nom => hy ; Acc => hom ; Gen => sy } ;
        a = Ag num per g ;
        isPron = True ;
